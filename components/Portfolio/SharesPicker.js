...
const SharesPicker = ({
  ...
  const handleSubmit = async () => {
    if (shares && shares !== '0') {
      ...
      if (edit) {
        await createNUpdateObj(`${currentUser.uid}/portfolio/${_id}`, obj);
      } else {
        await pushObj(`${currentUser.uid}/portfolio`, obj);
      }
      setMessageType('success');
      setMessage('Stock successfully added into your portfolio !');
      //onClose();
    } else {
      setMessageType('error');
      setMessage('Please enter amount or number of shares!');
    }
  };
